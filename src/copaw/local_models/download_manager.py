...
class ThreadProcessAdapter:
    """Process-like adapter backed by a worker thread."""

    def __init__(self, worker: threading.Thread):
        self._worker = worker
    ...
    def is_alive(self) -> bool:
        ...
        if not alive and self.returncode is None:
            self.returncode = 0
        return alive

    def join(self, timeout: float | None = None) -> None:
        self._worker.join(timeout=timeout)
        if not self._worker.is_alive() and self.returncode is None:
            self.returncode = 0
...
class ProcessDownloadController:
    ...
    def start(self, spec: ProcessDownloadTaskSpec) -> None:
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Falling back to thread-based downloader for %s: %s",
                    spec.process_name,
                    exc,
                )
                fallback_to_thread = True
                queue = ThreadQueue()
                worker_thread = threading.Thread(
                    target=self._run_worker_in_thread,
                    args=(spec, queue),
                    name=f"{spec.process_name}-worker",
                    daemon=True,
                )
                worker_thread.start()
                process = ThreadProcessAdapter(worker_thread)
            monitor_thread = threading.Thread(
            ...
            if fallback_to_thread:
                # Thread fallback cannot be forcefully interrupted the same
                # way as a subprocess-backed worker.
                logger.info(
                    "Thread fallback active for %s; cancellation may be limited.",
                    spec.process_name,
                )
    ...
    def cancel(self) -> None:
        with self._lock:
            ...
            if task is None or not task.process.is_alive():
                return
            if isinstance(task.process, ThreadProcessAdapter):
                raise RuntimeError(
                    "Current download runs in compatibility mode and "
                    "cannot be cancelled safely. Please wait for completion.",
                )
    ...
    def _run_worker_in_thread(
        ...
        try:
            spec.task.target(spec.task.payload, queue)
        except Exception as exc:  # pragma: no cover - defensive fallback
            queue.put(
                DownloadTaskResult(
                    status=DownloadTaskStatus.FAILED,
                    error=f"Download worker failed: {exc}",
                ).to_message(),
            )
