...
_wd_raw = (
    os.environ.get("MILU_WORKING_DIR", "").strip()
    or os.environ.get("COPAW_WORKING_DIR", "").strip()
    or "~/.milu"
)
_BOOTSTRAP_WORKING_DIR = Path(_wd_raw).expanduser().resolve()
_sec_raw = os.environ.get("MILU_SECRET_DIR", "").strip() or os.environ.get(
    "COPAW_SECRET_DIR",
    f"{_BOOTSTRAP_WORKING_DIR}.secret",
)
_BOOTSTRAP_SECRET_DIR = Path(_sec_raw).expanduser().resolve()
