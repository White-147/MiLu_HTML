...
def _working_dir_path() -> Path:
    raw = (
        EnvVarLoader.get_str("MILU_WORKING_DIR", "")
        or EnvVarLoader.get_str("milu_WORKING_DIR", "")
        or "~/.milu"
    )
    return Path(raw).expanduser().resolve()


WORKING_DIR = _working_dir_path()

SECRET_DIR = (
    Path(
        EnvVarLoader.get_str("MILU_SECRET_DIR", "")
...
# Chinese all-in-one documentation (relative to repository root, sibling of `src/`).
MILU_DOCS_ZH_ALL_DOCX_RELPATH = (
    "website/exported-docs/milu-docs-zh/milu-docs-zh-all.docx"
)
MILU_DOCS_ZH_ALL_MD_RELPATH = (
    "website/exported-docs/milu-docs-zh/milu-docs-zh-all.local.md"
)


def milu_docs_zh_all_docx_path() -> Path:
    """Absolute path to ``MILU_DOCS_ZH_ALL_DOCX_RELPATH`` in the MiLu repo."""
    repo_root = Path(__file__).resolve().parent.parent.parent
    return (repo_root / MILU_DOCS_ZH_ALL_DOCX_RELPATH).resolve()


def milu_docs_zh_all_markdown_path() -> Path:
    """Absolute path to ``MILU_DOCS_ZH_ALL_MD_RELPATH`` in the MiLu repo."""
    repo_root = Path(__file__).resolve().parent.parent.parent
    return (repo_root / MILU_DOCS_ZH_ALL_MD_RELPATH).resolve()


JOBS_FILE = EnvVarLoader.get_str("milu_JOBS_FILE", "jobs.json")

CHATS_FILE = EnvVarLoader.get_str("milu_CHATS_FILE", "chats.json")

# Builtin multi-agent profile: MiLu Q&A helper (ID kept for workspace path compatibility).
BUILTIN_QA_AGENT_ID = "milu_QA_Agent_0.1beta1"
...
TOKEN_USAGE_FILE = EnvVarLoader.get_str(
    "milu_TOKEN_USAGE_FILE",
    "token_usage.json",
)

CONFIG_FILE = EnvVarLoader.get_str("milu_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = EnvVarLoader.get_str("milu_HEARTBEAT_FILE", "HEARTBEAT.md")
...
LLAMA_CPP_DOWNLOAD_TRUST_ENV = EnvVarLoader.get_bool(
    "milu_LLAMA_CPP_DOWNLOAD_TRUST_ENV",
    False,
)
...
LOCAL_MODEL_DOWNLOAD_TRUST_ENV = EnvVarLoader.get_bool(
    "milu_LOCAL_MODEL_DOWNLOAD_TRUST_ENV",
    False,
)
...
DOCS_ENABLED = EnvVarLoader.get_bool("milu_OPENAPI_DOCS", False)

# Memory directory
MEMORY_DIR = WORKING_DIR / "memory"

# Custom channel modules (installed via `milu channels install`); manager
...
MEMORY_COMPACT_KEEP_RECENT = EnvVarLoader.get_int(
    "milu_MEMORY_COMPACT_KEEP_RECENT",
    3,
    min_value=0,
)
...
# Example: milu_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = EnvVarLoader.get_str("milu_CORS_ORIGINS", "").strip()

# LLM API retry configuration
LLM_MAX_RETRIES = EnvVarLoader.get_int(
    "milu_LLM_MAX_RETRIES",
    3,
    min_value=0,
)

LLM_BACKOFF_BASE = EnvVarLoader.get_float(
    "milu_LLM_BACKOFF_BASE",
    1.0,
    min_value=0.1,
)

LLM_BACKOFF_CAP = EnvVarLoader.get_float(
    "milu_LLM_BACKOFF_CAP",
    10.0,
    min_value=0.5,
)
...
try:
    TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS = max(
        float(
            os.environ.get("milu_TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS", "600"),
        ),
        1.0,
    )
except (TypeError, ValueError):
