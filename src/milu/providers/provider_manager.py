...
from milu.providers.provider import (
    ModelInfo,
    Provider,
    ProviderInfo,
)
from milu.providers.models import ModelSlotConfig
from milu.providers.openai_provider import OpenAIProvider
from milu.providers.anthropic_provider import AnthropicProvider
from milu.providers.gemini_provider import GeminiProvider
from milu.providers.ollama_provider import OllamaProvider
from milu.constant import SECRET_DIR
...
class ProviderManager:  # pylint: disable=too-many-public-methods
    ...
    def _init_builtins(self):
        self._add_builtin(PROVIDER_milu)
    ...
    def _migrate_copaw_local_to_milu_local(self):
        """Rename the legacy copaw-local provider to milu-local on disk."""
        old_path = self.builtin_path / "copaw-local.json"
        new_path = self.builtin_path / "milu-local.json"
        if old_path.exists() and not new_path.exists():
            try:
                with open(old_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                data["id"] = "milu-local"
                if data.get("name") in ("CoPaw Local", "copaw-local"):
                    data["name"] = "MiLu Local"
                ...
                old_path.unlink()
                logger.info(
                    "Migrated builtin provider copaw-local -> milu-local.",
                )
            except Exception as exc:
                logger.warning(
                    "Failed to migrate copaw-local provider: %s", exc,
                )
        ...
        if active_path.exists():
            try:
                ...
                if active_data.get("provider_id") == "copaw-local":
                    active_data["provider_id"] = "milu-local"
                    with open(active_path, "w", encoding="utf-8") as f:
                        json.dump(
                            active_data, f, ensure_ascii=False, indent=2,
                        )
                    logger.info(
                        "Migrated active_model provider_id "
                        "copaw-local -> milu-local.",
                    )
            except Exception as exc:
                logger.warning(
                    "Failed to migrate active_model provider_id: %s", exc,
                )

    def _migrate_legacy_providers(self):
        """Migrate from legacy providers.json format to the new structure."""
        self._migrate_copaw_local_to_milu_local()
    ...
    async def _resume_local_model(self, local_manager) -> None:
        ...
        local_models = self.get_provider("milu-local").extra_models
