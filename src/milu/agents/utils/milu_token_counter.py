# -*- coding: utf-8 -*-
"""Token counting utilities for milu using HuggingFace tokenizers.
...
if TYPE_CHECKING:
    from milu.config.config import AgentProfileConfig

logger = logging.getLogger(__name__)


class miluTokenCounter(HuggingFaceTokenCounter):
    """Token counter for milu with configurable tokenizer support.
...
class miluEstimateTokenCounter(HuggingFaceTokenCounter):
...
_token_counter_cache: dict[
    tuple,
    miluTokenCounter | miluEstimateTokenCounter,
] = {}


def get_milu_token_counter(
    ...
    Args:
        ...
        use_estimate: If True (default), returns a miluEstimateTokenCounter
            that uses only character-based estimation without loading a
            tokenizer. If False, returns a full miluTokenCounter backed
    ...
    if use_estimate:
        return miluEstimateTokenCounter(
        ...
        if config_key not in _token_counter_cache:
            _token_counter_cache[config_key] = miluTokenCounter(
