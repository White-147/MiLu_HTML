...
def get_visible_workspace_md_filenames(
    language: str | None,
    *,
    extra_filenames: list[str] | None = None,
) -> set[str]:
    """Return markdown filenames that should appear in the workspace UI.

    The base visible set comes from the current language template pack.
    Any explicitly enabled system prompt files are also included so custom
    prompt files remain visible when the system actively uses them.
    """
    agents_root = Path(__file__).resolve().parent.parent
    md_lang_dir = _resolve_md_lang_dir(agents_root, (language or "zh"))
    visible = {
        md_file.name for md_file in md_lang_dir.glob("*.md") if md_file.is_file()
    }
    if extra_filenames:
        visible.update(
            filename
