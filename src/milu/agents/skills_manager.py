...
def _write_json_atomic(path: Path, payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        try:
            ...
            # Some Windows/sandboxed environments block rename/delete while
            # still allowing direct overwrite of the destination file.
            write_text_fallback(path, serialized)
    finally:
        if temp_path is not None and temp_path.exists():
            try:
                temp_path.unlink(missing_ok=True)
            except PermissionError:
                logger.debug("Skipping temp cleanup for locked path: %s", temp_path)
...
def _extract_requirements(post: dict[str, Any]) -> SkillRequirements:
    ...
    elif "milu" in metadata:
        requires = metadata["milu"].get("requires", {})
    else:
...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return f"milu_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
...
def apply_skill_config_env_overrides(
    ...
    ``milu_SKILL_CONFIG_<SKILL_NAME>`` (JSON string).
    """
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="milu_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"milu_skill_stage_{skill_name}_"),
    )
