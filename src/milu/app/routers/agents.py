...
from ...agents.utils import get_visible_workspace_md_filenames
...
async def list_agent_files(
    ...
    visible_filenames = get_visible_workspace_md_filenames(
        ...
        extra_filenames=agent_config.system_prompt_files or [],
    )

    try:
        files = [
            MdFileInfo.model_validate(file)
            for file in workspace_manager.list_working_mds(visible_filenames)
        ]
        return files
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
...
def _initialize_agent_workspace(
    ...
    """Initialize agent workspace (similar to milu init --defaults)."""
