...
from ..utils.atomic_write import write_text_fallback
...
def _resolve_workspace_language(config, workspace_dir: Path) -> str:
    """Resolve workspace language from agent.json, then root config."""
    ...
    if agent_config_path.exists():
        try:
            ...
            logger.warning(
                "Failed to read workspace language from %s: %s",
                agent_config_path,
                e,
            )

    fallback = getattr(config.agents, "language", None)
    if isinstance(fallback, str) and fallback.strip():
        return fallback.strip()
    return "zh"


def _ensure_default_workspace_md_files(config, workspace_dir: Path) -> None:
    """Ensure default workspace runtime folders and md templates exist."""
    from .routers.agents import _ensure_heartbeat_file, _seed_workspace_md_files

    (workspace_dir / "sessions").mkdir(exist_ok=True)
    (workspace_dir / "memory").mkdir(exist_ok=True)
    (workspace_dir / "skills").mkdir(exist_ok=True)

    language = _resolve_workspace_language(config, workspace_dir)
    _seed_workspace_md_files(
        workspace_dir,
        language,
        builtin_qa_md_seed=False,
    )
...
def _do_migrate_legacy_workspace() -> bool:
    ...
    try:
        ...
        try:
            ...
            write_text_fallback(agent_config_path, serialized)
        ...
        if agent_config_tmp.exists():
            try:
                ...
                logger.debug(
                    "Skipping temp cleanup for locked path: %s",
                    agent_config_tmp,
                )
        raise
...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.milu or WORKING_DIR)
...
def _do_ensure_default_agent() -> None:
    ...
    _ensure_default_workspace_md_files(config, default_workspace)
...
def ensure_qa_agent_exists() -> None:
    ...
    ``milu_source_index``), and built-in tools are restricted (see
    ``build_qa_agent_tools_config``).
...
def _do_ensure_qa_agent() -> None:
    ...
    agent_config = AgentProfileConfig(
        ...
        description=(
            ...
            "milu_WORKING_DIR, and documentation. Prefer reading files "
