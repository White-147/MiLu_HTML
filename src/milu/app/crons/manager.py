...
class CronManager:
    ...
    async def run_job(self, job_id: str) -> None:
        ...
        if job.dispatch.channel == "console" and self._chat_manager is not None:
            session_id = (
                job.dispatch.target.session_id or f"cron:{job.id}"
            )
            user_id = job.dispatch.target.user_id or "cron"
            user_text = CronExecutor._extract_user_text(job)
            chat_name = user_text[:10] if user_text else job.name
            await self._chat_manager.get_or_create_chat(
                session_id, user_id, "console", name=chat_name,
            )
    ...
    async def _execute_once(self, job: CronJobSpec) -> None:
        ...
        async with rt.sem:
            ...
            # Push a cron_started notification so the frontend can
            # auto-navigate to the chat page for real-time streaming.
            if job.dispatch.channel == "console":
                ...
                await push_store_append(
                    session_id,
                    f"⏳ Cron job [{job.name}] started",
                    msg_type="cron_started",
                )
