...
from .process_utils import (
    ...
    _is_milu_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/milu/json"
...
class InstallInfo:
    """Information about the current milu installation."""
...
class RunningServiceInfo:
    """Detected milu service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published milu version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest milu version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest milu version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest milu version.",
        )
    return version
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("milu")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running milu HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local milu service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_milu_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a milu HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running milu service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"milu service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `milu shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING milu SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `milu shutdown` will forcibly terminate the current "
        "milu backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `milu shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `milu shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "milu"]
    ...
    click.echo("")
    click.echo("Running `milu shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `milu shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`milu shutdown` failed. Please stop the running milu "
            "service manually before running `milu update`.",
        )
...
def _build_upgrade_command(
    ...
    package_spec = f"milu=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from milu.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[milu] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo(f"[milu] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[milu] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[milu] Update completed successfully.")
        click.echo(
            "[milu] Please restart any running milu service "
            "to use the new version.",
        )
    else:
        click.echo(f"[milu] Update failed with exit code {return_code}.")
        click.echo(
            "[milu] Please fix the error above and run "
            "`milu update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade MiLu in the current Python environment."""
    ...
    if version_check is False:
        click.echo("MiLu is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `milu update`, or rerun "
                "without `--yes` to confirm a forced `milu shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                "Detected "
                f"{_running_service_display(running)} after `milu shutdown`. "
                "Please stop it manually before running `milu update`.",
            )

    if not yes and not click.confirm(
        f"Update MiLu to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
