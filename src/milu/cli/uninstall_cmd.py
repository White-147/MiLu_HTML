# -*- coding: utf-8 -*-
"""milu uninstall — remove the MiLu environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove MiLu/milu legacy PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# milu" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# milu\nexport PATH=\"\$HOME/\.milu/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove MiLu environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL MiLu data in {wd}")
    else:
        click.echo(
            "This will remove the MiLu Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("MiLu uninstalled. Please restart your terminal.")
