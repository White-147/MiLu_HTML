...
def _matches_milu_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a milu CLI invocation."""
    ...
    return any(
        ...
        for pattern in (
            ...
            f'milu.exe" {subcommand}',
            f"milu.exe {subcommand}",
        )
    )


def _is_milu_service_command(command: str) -> bool:
    """Return whether the command line looks like a local milu app."""
    return _matches_milu_cli_command(command, "app")


def _is_milu_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a milu CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "milu" or _matches_milu_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local milu service."""
    if not host:
        return []
