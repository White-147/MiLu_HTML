...
def chats_group() -> None:
    ...
    \b
    Common examples:
      milu chats list                    # List all chats
      milu chats list --user-id alice    # Filter by user
      milu chats get <chat_id>           # View details
      milu chats create --session-id s1 --user-id u1
      milu chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      milu chats list
      milu chats list --user-id alice
      milu chats list --channel discord
      milu chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `milu chats list`.
...
def create_chat(
    ...
    Inline creation examples:
      milu chats create --session-id "discord:alice" \\
...
def update_chat(
    ...
    \b
    Examples:
      milu chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
    payload = {"name": name}
...
def delete_chat(
    ...
    \b
    Examples:
      milu chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
