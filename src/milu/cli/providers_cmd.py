    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'milu[local]'",
                fg="red",
            ),
        )
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'milu models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'milu models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'milu models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'milu models add-model' to add models, "
        "then 'milu models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      milu models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
    ...
    click.echo(
        ...
        "  milu models set-llm  (select 'milu-local' provider)",
    )
...
def list_local_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'milu models download <repo_id>' to download one.")
        return
