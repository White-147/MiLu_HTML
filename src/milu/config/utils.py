...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize legacy ~/.milu-bound paths to current WORKING_DIR.

    This keeps milu_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.milu/media" or
    "/Users/x/.milu/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.milu"
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    milu.channels), filtered by milu_ENABLED_CHANNELS or
    milu_DISABLED_CHANNELS when set.

    * milu_ENABLED_CHANNELS — whitelist (only these channels are active).
    * milu_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, milu_ENABLED_CHANNELS takes precedence.
...
def is_running_in_container() -> bool:
    ...
    Prefer env milu_RUNNING_IN_CONTAINER (1/true/yes) at call time so
