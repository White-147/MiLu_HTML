...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is MiLu. How can I help you?"
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # This keeps milu_WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.milu/media".
    try:
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default MiLu agent",
    ...
    # When milu_WORKING_DIR is customized, historical data may still exist
    # under "~/.milu".
    old_workspace = Path("~/.milu").expanduser().resolve()
