...
TELEMETRY_ENDPOINT = "https://milu-telemetry-xissagieap.cn-hangzhou.fcapp.run"
...
def _detect_install_method() -> str:
    """Detect how milu was installed based on environment signals."""
    import os

    if os.environ.get("milu_RUNNING_IN_CONTAINER", "").lower() in (
        "1",
        "true",
        "yes",
    ):
        return "docker"
    if os.environ.get("milu_DESKTOP_APP", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def get_system_info() -> dict[str, Any]:
    ...
    - milu_version: milu version string
    - install_method: How milu was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running milu (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as milu_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "milu_version": _safe_get(lambda: milu_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current milu version string."""
    try:
        from ..__version__ import __version__ as milu_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when milu is upgraded (or downgraded) to a version
    that hasn't been collected before.

    Args:
        working_dir: Path to milu working directory
    ...
    try:
        ...
        # v1.1 compat: single milu_version field
        return marker_data.get("milu_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("milu_version", "")
                    if old_ver:
