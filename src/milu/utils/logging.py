# -*- coding: utf-8 -*-
"""Logging setup for milu: console output and optional file handler."""
import io
import logging
...
_milu_LOG_MAX_BYTES = 5 * 1024 * 1024  # 5 MiB
_milu_LOG_BACKUP_COUNT = 3
...
def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (milu), not deps."""
    ...
    # Only attach handler to our namespace so only milu.* logs are printed.
...
def add_milu_file_handler(log_path: Path) -> None:
    """Add a file handler to the MiLu logger for daemon logs.
    ...
    Args:
        log_path: Path to the log file (e.g. WORKING_DIR / "milu.log").
    """
    ...
    if is_windows_or_linux:
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            log_path,
            encoding="utf-8",
            maxBytes=_milu_LOG_MAX_BYTES,
            backupCount=_milu_LOG_BACKUP_COUNT,
        )
